# 👋 Hi there, I'm Yasmine Meftah

I'm a second-year student at **IIM Digital School (Paris – La Défense)**, majoring in **Coding & Digital Innovation**.  
I'm passionate about **web development**, **data engineering**, **IoT**, and **creative tech projects** that mix design, logic, and storytelling.

---

### 💼 Currently

- 🚀 **Internship @ Amplegest** — applying my development skills in a professional environment in **Finance**
- 🎓 Second-year student @ IIM Digital School

---

### 🌱 Currently Learning

- **Symfony** & **PHP MVC** architecture  
- **React / Next.js** and modern front-end workflows  
- **FastAPI** & **Python** for AI and data processing
- **PHP**
- and more...

---

### 🛠️ Projects I'm Working On

- **Curiositiz** → *Family activity marketplace platform* (auth, reviews, filters, chat system) — 🎉 **Live for a real client!** 👉 [curiositiz.com](https://curiositiz.com/)
- **Website Security** → *Securing a website* (authentication, validation, CSRF protection, error handling)
- **Tracking Tool @ Amplegest** → *Internal monitoring/reporting tool built during my internship*

---

### 📫 How to Reach Me

- 📧 **Email:** [yasmine.meftah@edu.iim.fr](mailto:yasmine.meftah@edu.iim.fr)  
- 💼 **LinkedIn:** [linkedin.com/in/yasminemeftah](https://linkedin.com/in/yasminemeftah)
- 💻 **Portfolio:** [portfolio-yasmine-meftah.onrender.com](https://portfolio-yasmine-meftah.onrender.com/)

---

### 😼 Fun Fact

When I'm not coding, I love **video games**, **K-pop**,**mangas** and **drawing** 🎨
